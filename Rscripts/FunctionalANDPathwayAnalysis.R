...
# this can be buggy, you may have to try and connect multiple times and/or use different mirrors
ensembl_mirror <- useEnsembl(biomart = "ensembl", mirror = "uswest")
mmus <- useMart("ensembl", dataset = "mmusculus_gene_ensembl", host = "https://dec2021.archive.ensembl.org") # get mus ensmebls
urop = useMart("ensembl", dataset = "uparryii_gene_ensembl", host = "https://dec2021.archive.ensembl.org") # get urocitellus ensemsbls
...
dge = as.data.frame(vobjDream$E) # create gene expression object to grab all background genes
...
# Change gene list to acquire specific IDs for what pairwise comparison you are interested in
# e.g., here we use 'earlyHibSexDWNGenes' to convert all downregulated genes at ealry hibernation 
# between the sexes
Genes = bitr(earlyHibSexDWNGenes, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")
...
MaleTreatNEW = read.csv("../DE_genes/Male_LHvsEH_sig.csv")
...
# Female 2W to LH
FemaleTreatNEW = read.csv("../DE_genes/Female_LHvsEH_sig.csv")
...
write.csv(FemaleTreatGOUP, 'FemaleTreatGOUP.csv')
write.csv(AGSbackground, 'AGSbackground.csv')
...
run_gost_and_process_results <- function(gene_list) {
  ...
  result_df <- data.frame(Term.ID = GORes$result$term_id,
                          ...
                          geneid = GORes$result$intersection,
                          P.value = GORes$result$p_value,
                          significant = GORes$result$source_order,
                          Source = GORes$result$source, 
                          Term.Size = GORes$result$term_size, 
                          Precision = GORes$result$precision,
                          intersection_size = GORes$result$intersection_size,
...
earlyHibSexNEW = read.csv("../DE_genes/lateHib_MvsF_sig.csv")
